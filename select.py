...
class VideoInputSelect(CoordinatorEntity[BluStreamCoordinator], SelectEntity): # pyright: ignore[reportIncompatibleVariableOverride] pylint: disable=abstract-method
    ...
    def __init__(
        ...
        self._select = getattr(output, "input_port", -1)
        self._entry = entry

        output_port = getattr(output, "output_port", "unknown")
        output_name = getattr(output, "name", f"Output {output_port}").replace("_", " ")

        self._attr_name = f"Input Select for {output_name}"
        self._attr_unique_id = f"{DOMAIN}_output_{output_port}_input"
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if this_output is not None:
            self._select = getattr(this_output, "input_port", -1)
        self.async_write_ha_state()

    @property
    def options(self) -> list[str] | None: # pyright: ignore[reportIncompatibleVariableOverride]
        """Return a set of selectable options."""
        input_names = getattr(self._coordinator.data, "input_names", {})
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        input_names = getattr(self._coordinator.data, "input_names", {})
        input_id = next((k for k, v in input_names.items() if v == option), None)
